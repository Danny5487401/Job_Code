...
def upload(request):
    ...
    if request.method == 'POST' and not len(request.FILES) is 0 and request.FILES['excel_file']:
        ...
        importer = request.user.username
        logger.info("importer= {}".format(importer))
        ...
        load_excel.load_excel(PROJECT_DIR + uploaded_file_url , importer)
...
def resume_statistic(request, post_id):
    ...
    status_id = request.META.get('HTTP_ORDOV_STATUS_ID', '0')
    status_id = int(status_id)
    print('debug_status_id:')
    print(status_id)
    # if status_id == 0:
    #     queryset_waitting = Resume.objects.exclude(interview__post__id=post_id)
    #
    # elif status_id > 0:
    #     queryset_waitting = Resume.objects.filter(interview__status=0, interview__post__id=post_id,
    #                                      interview__is_active=True)
    # elif status_id < 0:
    #     queryset_waitting = Resume.objects.filter(interview__status__gte=0, interview__post__id=post_id,
    #                                      interview__is_active=False)
    # else:
    #     queryset_waitting = Resume.objects.all()
    ...
    queryset_waitting = queryset_waitting.filter(~models.Q(hunting_status=1))
    ...
    if userProfile.user_type == "Manager":
        ...
        for i in range(1, 9):
            ...
            logger.info('当前查询的status i: {}'.format(i))
            queryset = Resume.objects.filter(interview__status=i, interview__post__id=post_id,
                                             interview__is_active=True)
            queryset = FilterResumeByPostRequest(queryset, post_id)
            queryset = queryset.distinct("id")
            resume_cnt1 = queryset.count()

            logger.info("i = {} , resume_cnt1 = {}".format(i, resume_cnt1))


            queryset2 = Resume.objects.filter(interview__status=i, interview__post__id=post_id,
                                                  hunting_status=1,
                                                  interview__is_active=True)


            queryset2 = FilterResumeByPostRequest(queryset2, post_id)
            queryset2 = queryset2.distinct("id")
            resume_cnt2 = queryset2.count()

            logger.info("i = {} , resume_cnt2 = {}".format(i, resume_cnt2))
            if i >= 3:
                resume_cnt2 = 0
            resume_cnt = resume_cnt1 - resume_cnt2

            logger.info("i = {} , result ,  resume_cnt = {}".format(i, resume_cnt2))

            if i == 1:
                status1_cnt = resume_cnt
                interviews_status_filters.append(resumes_waitting + resume_cnt)
            elif i == 2:
                interviews_status_filters.append(resumes_waitting + status1_cnt + resume_cnt)
            else:
                interviews_status_filters.append(resume_cnt)

        # 不合适的也要特殊计算，
        queryset3 = Resume.objects.filter(interview__status__gte=0, interview__post__id=post_id,
                                          interview__is_active=False)

        # queryset3 = queryset3.filter(~models.Q(hunting_status=1))
        queryset3 = FilterResumeByPostRequest(queryset3, post_id)
        # queryset3 = queryset3.distinct("id")
        interviews_status_filters.append(queryset3.count())
    else:
        status1_cnt = 0
        ...
        for i in range(1, 9):
            try:
                ...
                if permission:
                    ...
                    logger.info('子用户->当前查询的status i: {}'.format(i))
                    queryset = Resume.objects.filter(interview__status=i, interview__post__id=post_id,
                                                     interview__is_active=True)
                    queryset = FilterResumeByPostRequest(queryset, post_id)
                    queryset = queryset.distinct("id")
        ...
        # 不合适的也要特殊计算，
        queryset4 = Resume.objects.filter(interview__status__gte=0, interview__post__id=post_id,
                                          interview__is_active=False)
        # queryset4 = queryset4.filter(~models.Q(hunting_status=1))
        queryset4 = FilterResumeByPostRequest(queryset4, post_id)
        # queryset3 = queryset3.distinct("id")
        interviews_status_filters.append(queryset4.count())
